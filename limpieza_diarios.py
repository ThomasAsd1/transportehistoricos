...
# Configura el prefijo con el formato correcto
prefix = f'{today}/'

# Obtén los blobs (archivos) del bucket de origen con el prefijo correcto
blobs = list(client.list_blobs(bucket_origen_name, prefix=prefix))

# Verifica cuántos blobs se encontraron
num_blobs = len(blobs)
...
for blob in blobs:
    ...
    if blob_name.endswith('.json'):
        ...
        ruta_json = f'gs://{bucket_origen_name}/{blob_name}'
        ...
        for blob_destino in client.list_blobs(bucket_destino_name, prefix=ruta_carpeta_destino):
        ...
        if not existe_en_destino:
            ...
            try:
                ...
                shapes_blob_destino = client.bucket(bucket_destino_name).blob(f'{ruta_carpeta_destino}shapes.csv')
                shapes_blob_destino.upload_from_string(shapes_csv, content_type='text/csv')

                stops_csv = stops.to_csv(index=False)
                stops_blob_destino = client.bucket(bucket_destino_name).blob(f'{ruta_carpeta_destino}stops.csv')
                stops_blob_destino.upload_from_string(stops_csv, content_type='text/csv')

                routes_csv = routes.to_csv(index=False)
                routes_blob_destino = client.bucket(bucket_destino_name).blob(f'{ruta_carpeta_destino}routes.csv')
